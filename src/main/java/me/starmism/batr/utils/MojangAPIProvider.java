...
public class MojangAPIProvider {
    ...
    public static String getUUID(final String pName) {
        ...
        try {
            final URL url = new URL("https://api.mojang.com/users/profiles/minecraft/" + pName);
            ...
            if (p != null && !p.id.isEmpty()) {
                BATR.getInstance().getLogger().severe("Got ID");
                return p.id;
            }
        } catch (IOException e) {
            BATR.getInstance().getLogger().log(Level.CONFIG, "Can't retrieve UUID from mojang servers", e);
        } finally {
            if (reader != null) {
                try {
                    reader.close();
    ...
    public static List<String> getPlayerNameHistory(final String pName) throws RuntimeException {
        ...
        try {
            ...
            for (final Map<String, Object> entry :
                    (Set<Map<String, Object>>) gson.fromJson(content.toString(), new TypeToken<Set<Map<String, Object>>>() {
                    }.getType())) {
