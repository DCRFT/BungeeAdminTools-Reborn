...
public class ModulesManager {
	...
	public ModulesManager() {
		log = BATR.getInstance().getLogger();
		...
		for (final IModule module : modules.keySet()) {
			...
			if (!module.getName().equals("core")) {
				...
				if (module instanceof BanConfig) {
					isEnabled = module.getConfig().get(BanConfig.ENABLED);
				} else if (module instanceof MuteConfig) {
					isEnabled = module.getConfig().get(MuteConfig.ENABLED);
				} else if (module instanceof CommentConfig) {
					isEnabled = module.getConfig().get(CommentConfig.ENABLED);
				} else {
					isEnabled = module.getConfig().get(KickConfig.ENABLED);
				}
				if (!isEnabled) {
					continue;
				}
			...
			if (module.load()) {
				...
				if (module instanceof Listener) {
					ProxyServer.getInstance().getPluginManager().registerListener(BATR.getInstance(), (Listener) module);
				}

				for (final BATCommand cmd : module.getCommands()) {
					cmdsModules.put(cmd.getName(), module);
					ProxyServer.getInstance().getPluginManager().registerCommand(BATR.getInstance(), cmd);
				}
	...
	public void unloadModules() {
		...
		ProxyServer.getInstance().getPluginManager().unregisterCommands(BATR.getInstance());
		modules.clear();
	}
