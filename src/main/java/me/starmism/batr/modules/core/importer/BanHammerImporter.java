    ...
    protected void importData(final ProgressCallback<ImportStatus> progressionCallback, String... additionalsArgs) throws Exception {
        ...
        try (Connection conn = BATR.getConnection()) {
            ...
            if (!tableFound) {
                ...
                if (new File(BATR.getInstance().getDataFolder(), "banhammer.db").exists()) {
                    ...
                    throw new RuntimeException("No BanHammer tables was found in the MySQL database. "
                            + "If you used a .db file with BanHammer, please put your file into BATR folder and rename it 'banhammer.db'");
                }
            }
            ...
            if (tableFound) {
                try (Connection connBH = (banHammerOnMysql)
                        ? conn
                        : DriverManager.getConnection("jdbc:sqlite:" + BATR.getInstance().getDataFolder().getAbsolutePath() + File.separator
                        + "banhammer.db")) {
