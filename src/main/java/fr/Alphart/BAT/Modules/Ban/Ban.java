...
public class Ban implements IModule, Listener {
    ...
    public Ban() {
        config = SettingsManager.from(Path.of(BAT.getInstance().getDataFolder().getPath(), "ban.yml"))
            ...
            if (Utils.validIP(bannedEntity)) {
                ...
                for (final ProxiedPlayer player : ProxyServer.getInstance().getPlayers()) {
                    if (Utils.getPlayerIP(player).equals(bannedEntity) && (GLOBAL_SERVER.equals(server) || server.equalsIgnoreCase(player.getServer().getInfo().getName()))) {
                        BAT.kick(player, format("wasBannedNotif", new String[]{reason}));
                    }
                }
                ...
                if (BAT.getInstance().getRedis().isRedisEnabled()) {
                    for (final UUID pUUID : RedisBungee.getApi().getPlayersOnline()) {
                        if (RedisBungee.getApi().getPlayerIp(pUUID).toString().equals(bannedEntity) && (GLOBAL_SERVER.equals(server) || server.equalsIgnoreCase(RedisBungee.getApi().getServerFor(pUUID).getName()))) {
                            BAT.getInstance().getRedis().sendGKickPlayer(pUUID, format("wasBannedNotif", new String[]{reason}));
                        }
                    }
                }
    ...
    public String unBan(final String bannedEntity, final String server, final String staff, final String reason) {
        ...
        try (Connection conn = BAT.getConnection()) {
            ...
            if (Utils.validIP(bannedEntity)) {
                ...
                return format("unbanBroadcast", new String[]{ip, staff, server, reason});
            }
            ...
            // Otherwise it's a player
                ...
                return format("unbanBroadcast", new String[]{pName, staff, server, reason});
            }
        } catch (final SQLException e) {
            return DataSourceHandler.handleException(e);
        } finally {
        ...
        if (isBan(player, target)) {
            if (target.equals(player.getPendingConnection().getListener().getDefaultServer())) {
                ...
                if (e.getPlayer().getServer() == null) {
                    ...
                    ProxyServer.getInstance().getScheduler().schedule(BAT.getInstance(), () ->
