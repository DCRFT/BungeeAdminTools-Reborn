...
public class Mute implements IModule, Listener {
    ...
    public Mute() {
        config = SettingsManager.from(Path.of(BAT.getInstance().getDataFolder().getPath(), "mute.yml"))
        ...
        try (Connection conn = BAT.getConnection()) {
            if (Utils.validIP(mutedEntity)) {
                        ...
                        if (RedisBungee.getApi().getPlayerIp(pUUID).equals(ip)) {
                            ...
                            if (server.equals(GLOBAL_SERVER) || RedisBungee.getApi().getServerFor(pUUID).getName().equalsIgnoreCase(server)) {
                                ...
                                if (player != null) {
                                    ...
                                    BAT.getInstance().getRedis().sendMessagePlayer(pUUID, TextComponent.toLegacyText(I18n.formatPrefix("wasMutedNotif", new String[]{reason})));
                                }
                            }
                        }
                    }
                ...
                if (expirationTimestamp > 0) {
                    return format("muteTempBroadcast", new String[]{ip, FormatUtils.getDuration(expirationTimestamp),
                            staff, server, reason});
                } else {
                    return format("muteBroadcast", new String[]{ip, staff, server, reason});
                }
            }
            ...
            // Otherwise it's a player
                ...
                if (expirationTimestamp > 0) {
                    return format("muteTempBroadcast", new String[]{pName, FormatUtils.getDuration(expirationTimestamp),
                            staff, server, reason});
                } else {
                    return format("muteBroadcast", new String[]{pName, staff, server, reason});
                }
    ...
    public String unMute(final String mutedEntity, final String server, final String staff, final String reason) {
        ...
        try (Connection conn = BAT.getConnection()) {
            ...
            if (Utils.validIP(mutedEntity)) {
                ...
                return format("unmuteBroadcast", new String[]{ip, staff, server, reason});
            }
            ...
            // Otherwise it's a player
                ...
                if (player != null) {
                    ...
                    if (ANY_SERVER.equals(server) || GLOBAL_SERVER.equals(server) || (pServer != null && pServer.getName().equalsIgnoreCase(server))) {
                        BAT.getInstance().getRedis().sendMuteUpdatePlayer(pUUID, server);
                        BAT.getInstance().getRedis().sendMessagePlayer(pUUID, TextComponent.toLegacyText(I18n.formatPrefix("wasUnmutedNotif", new String[]{reason})));
                    }
                }

                return format("unmuteBroadcast", new String[]{pName, staff, server, reason});
            }
        } catch (final SQLException e) {
            return DataSourceHandler.handleException(e);
        } finally {
    ...
    public void onPlayerChat(final ChatEvent e) {
        ...
        if (e.isCommand()) {
            ...
            for (final String forbiddenCmd : config.get(MuteConfig.forbiddenCmds)) {
    ...
    public static class MuteConfig implements SettingsHolder {
        ...
        public static final Property<Set<String>> forbiddenCmds = Property.create(Set.of("msg"));
