...
public abstract class BATCommand extends net.md_5.bungee.api.plugin.Command implements TabExecutor {
    ...
    public void handleCommandException(final CommandSender sender, final Exception exception) {
        if (exception instanceof IllegalArgumentException) {
            ...
            sender.sendMessage(BAT.__("&cAn error occurred with the translation. Key involved : &a" + ((MissingResourceException) exception).getKey()));
        } else {
            sender.sendMessage(BAT.__("A command error happened! Please check the console."));
    ...
    public void execute(final CommandSender sender, final String[] args) {
        ...
        final boolean confirmedCmd = !BAT.getInstance().getConfiguration().get(Configuration.confirmCommand) || CommandQueue.isExecutingQueueCommand(sender);
        try {
    ...
    public void mustConfirmCommand(final CommandSender sender, final String command, final String message) {
        final String cmdToConfirm = (BAT.getInstance().getConfiguration().get(Configuration.simpleAliasesCommands).get("confirm"))
                ? "confirm" : "bat confirm";
