...
public class BAT extends Plugin {
    ...
    public void onEnable() {
        ...
        if (config.get(Configuration.debugMode)) {
            try {
        ...
        loadDB((dbState, throwable) -> {
            if (dbState) {
                ...
                redis = new RedisUtils(config.get(Configuration.redisSupport));
    ...
    public void loadDB(final Callback<Boolean> dbState) {
        ...
        // (if it isn't already in the lib folder) and load it
            ...
            getLogger().warning("It is strongly *NOT RECOMMENDED* to use SQLite with BAT,"
