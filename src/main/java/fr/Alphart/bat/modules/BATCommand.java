...
public abstract class BATCommand extends net.md_5.bungee.api.plugin.Command implements TabExecutor {
    ...
    public void handleCommandException(final CommandSender sender, final Exception exception) {
        if (exception instanceof IllegalArgumentException) {
            ...
            sender.sendMessage(BAT.convertStringToComponent("&cAn error occurred with the translation. Key involved: &a" + ((MissingResourceException) exception).getKey()));
        } else {
            sender.sendMessage(BAT.convertStringToComponent("A command error happened! Please check the console."));
            BAT.getInstance().getLogger().severe("A command error happened! Please report this stacktrace :");
    ...
    public void execute(final CommandSender sender, final String[] args) {
        ...
        final boolean confirmedCmd = !BAT.getInstance().getConfiguration().get(Configuration.CONFIRM_COMMAND) || CommandQueue.isExecutingQueueCommand(sender);
        try {
    ...
    public Iterable<String> onTabComplete(final CommandSender sender, final String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(BAT.convertStringToComponent("Add the first letter to autocomplete"));
            return result;
        }
    ...
    public void mustConfirmCommand(final CommandSender sender, final String command, final String message) {
        final String cmdToConfirm = (BAT.getInstance().getConfiguration().get(Configuration.SIMPLE_ALIASES_COMMANDS).get("confirm"))
                ? "confirm" : "bat confirm";
