...
import fr.Alphart.bat.Configuration;
...
public class CoreCommand extends BATCommand {
    ...
    public CoreCommand(final Core coreModule) {
        super("bat", "", "", null);
        final Map<String, Boolean> simpleAliasesCommands = BAT.getInstance().getConfiguration().get(Configuration.SIMPLE_ALIASES_COMMANDS);
        subCmd = new HashMap<>();
    ...
    public static class ModulesCmd extends BATCommand {
        ...
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
                throws IllegalArgumentException {
            sender.sendMessage(BAT.convertStringToComponent("The loaded modules are:&a"));
            ...
            if (sb.length() == 0) {
                sender.sendMessage(BAT.convertStringToComponent("&cThere aren't any loaded modules!"));
            } else {
    ...
    public static class ReloadCmd extends BATCommand {
        ...
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
                throws IllegalArgumentException {
            sender.sendMessage(BAT.convertStringToComponent("Starting reload ..."));
            BAT.getInstance().getConfiguration().reload();
            I18n.reload();
            BAT.getInstance().getModules().unloadModules();
            BAT.getInstance().getModules().loadModules();
            sender.sendMessage(BAT.convertStringToComponent("Reload successfully executed ..."));
        }
    }
                    ...
                    switch (args[1]) {
                        case "ban" -> {
                            ...
                            if (!bans.isEmpty()) {
                                ...
                                message.add(BAT.convertStringToComponent((!Utils.validIP(entity))
                    ...
                    switch (args[1]) {
                        case "ban" -> {
                            ...
                            if (!bans.isEmpty()) {
                                ...
                                message = new ArrayList<>();
                                message.add(BAT.convertStringToComponent("&b" + entity + "&e has never performed any operation concerning ban."));
                            }
                        }
                        case "mute" -> {
                            ...
                            if (!mutes.isEmpty()) {
                                ...
                                message = new ArrayList<>();
                                message.add(BAT.convertStringToComponent("&b" + entity + "&e has never performed any operation concerning mute."));
                            }
                        }
                        case "kick" -> {
                            ...
                            if (!kicks.isEmpty()) {
                                ...
                                message = new ArrayList<>();
                                message.add(BAT.convertStringToComponent("&b" + entity + "&e has never performed any operation concerning kick."));
                            }
                        }
                        case "comment" -> {
                            ...
                            if (!comments.isEmpty()) {
                                ...
                                message = new ArrayList<>();
                                message.add(BAT.convertStringToComponent("&b" + entity + "&e has never performed any operation concerning comment."));
                            }
                        }
                        default -> throw new InvalidModuleException("Module not found or invalid");
                    }
    ...
    public static class ImportCmd extends BATCommand {
        ...
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
                throws IllegalArgumentException {
            checkArgument(BAT.getInstance().getConfiguration().get(Configuration.MYSQL_ENABLED), "You must use MySQL in order to use the import function.");
            final String source = args[0];

            final Importer importer = importers.get(source);
            if (importer != null) {
                sender.sendMessage(BAT.convertStringToComponent("BAT will be disabled during the import ..."));
                BAT.getInstance().getModules().unloadModules();
                ...
                importer.startImport(new ProgressCallback<>() {
                    ...
                    public void done(ImportStatus result, Throwable throwable) {
                        if (throwable != null) {
                            if (throwable instanceof RuntimeException) {
                                sender.sendMessage(BAT.convertStringToComponent("An error (" + throwable.getMessage()
                                        + ") has occurred during the import. Please check the logs"));
                            } else {
                                sender.sendMessage(BAT.convertStringToComponent("An error has occured during the import. Please check the logs"));
                                BAT.getInstance().getLogger().severe("An error has occurred during the import of data from " + source
                                        + ". Please report this :");
                            }
                            throwable.printStackTrace();
                        } else {
                            sender.sendMessage(BAT.convertStringToComponent("Congratulations, the migration is finished. &a"
                                    + result.getConvertedEntries() + " entries&e were converted successfully."));
                        }
                        BAT.getInstance().getModules().loadModules();
                        sender.sendMessage(BAT.convertStringToComponent("BAT is now re-enabled ..."));
                    }

                    @Override
                    public void onProgress(ImportStatus progressStatus) {
                        sender.sendMessage(BAT.convertStringToComponent("&a" + new DecimalFormat("0.00").format(progressStatus.getProgressionPercent())
                    ...
                    public void onMinorError(String errorMessage) {
                        sender.sendMessage(BAT.convertStringToComponent(errorMessage));
                    }
    ...
    public static class BackupCmd extends BATCommand {
        public BackupCmd() {
            super("backup", "", "Backup the BAT's data from the mysql database into a file", "bat.backup");
        }
        ...
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
            ...
            sender.sendMessage(BAT.convertStringToComponent("Starting backup of BAT datas ..."));
            BAT.getInstance().getDsHandler().generateMysqlBackup((result, throwable) -> sender.sendMessage(BAT.convertStringToComponent(result)));
        }
    }
    ...
    public static class MigrateCmd extends BATCommand {
        ...
        @Override
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd) throws IllegalArgumentException {
            boolean isImportSimpleAlias = BAT.getInstance().getConfiguration().get(Configuration.SIMPLE_ALIASES_COMMANDS).get("import");
