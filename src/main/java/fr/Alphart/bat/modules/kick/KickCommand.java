...
public class KickCommand extends CommandHandler {
    ...
    public static class KickCmd extends BATCommand {
        ...
        public void onCommand(final CommandSender sender, final String[] args, final boolean confirmedCmd)
            ...
            checkArgument(args.length != 1 || !BAT.getInstance().getConfiguration().get(Configuration.MUST_GIVE_REASON),
            ...
            if (player != null) {
                ...
                checkArgument(
                        ...
                        format("cantKickDefaultServer", new String[]{pName}));

                checkArgument(
                        PermissionManager.canExecuteAction(Action.KICK, sender, player.getServer().getInfo().getName()),
                        I18n.format("noPerm"));

                checkArgument(!PermissionManager.isExemptFrom(Action.KICK, pName), I18n.format("isExempt"));
                ...
                if (defaultServer == null || pServer.equals(defaultServer)) {
                    throw new IllegalArgumentException(format("cantKickDefaultServer", new String[]{pName}));
                }

                checkArgument(PermissionManager.canExecuteAction(Action.KICK, sender, pServer), I18n.format("noPerm"));
