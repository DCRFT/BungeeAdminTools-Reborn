...
public class Mute implements IModule, Listener {
    ...
    public void onServerConnect(final ServerConnectedEvent e) {
        ...
        if (muteState == -1) {
            ...
            BAT.getInstance().getProxy().getScheduler().schedule(BAT.getInstance(),
            ...
            if (muteMessage != null) {
                if (muteMessage.getValue() != null) {
                    if (muteMessage.getValue().getTime() >= System.currentTimeMillis()) {
                        return BAT.convertStringToComponent(muteMessage.getKey().replace("{expiration}", FormatUtils.getDuration(muteMessage.getValue().getTime())));
                    }
                    ...
                    return BAT.convertStringToComponent(muteMessage.getKey());
                }
            }
