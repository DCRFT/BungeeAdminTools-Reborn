...
public class MuteCommand extends CommandHandler {
	...
	public static void handleMuteCommand(final BATCommand command, final boolean global, final boolean ipMute,
        ...
        checkArgument(
                    !reason.equalsIgnoreCase(IModule.NO_REASON) || !BAT.getInstance().getConfiguration().get(Configuration.MUST_GIVE_REASON),
		...
		checkArgument(!PermissionManager.isExemptFrom(Action.MUTE, target), I18n.format("isExempt"));
	...
	public static void handleUnmuteCommand(final BATCommand command, final boolean global, final boolean ipUnmute,
		...
		checkArgument(
				...
				(IModule.ANY_SERVER.equals(server) ? format("notMutedAny", formatArgs) : ((ipUnmute) ? format("notMutedIP",
						formatArgs) : format("notMuted", formatArgs))));
