...
public class I18n {
    ...
    private I18n() {
        ...
        try (final InputStream translationsZip = BAT.getInstance().getResourceAsStream("translations.zip");
             final ZipInputStream zip = new ZipInputStream(Objects.requireNonNull(translationsZip, "translationsZip"))) {
            ZipEntry zipEntry;
            Files.createDirectories(BAT.getInstance().getDataFolder().toPath().resolve("lang"));
            ...
            BAT.getInstance().getLogger().severe("Could not output the default jar language files.");
            e.printStackTrace();
        }

        // Get the language file associated with the language setting in the config.
        final Locale locale = new Locale(BAT.getInstance().getConfiguration().get(Configuration.LANGUAGE));
        try {
            bundle = new PropertyResourceBundle(new FileReader(BAT.getInstance().getDataFolder()
    ...
    public static String getString(final String key) throws IllegalArgumentException {
        ...
        try {
            ...
            BAT.getInstance().getLogger().info("Incorrect translation key: " + key);
    ...
    public static BaseComponent[] formatPrefix(final String message, final String[] formatObject) {
        try {
            final MessageFormat mf = new MessageFormat(getString(message));
            return BAT.convertStringToComponent(mf.format(preprocessArgs(formatObject)));
        } catch (final IllegalArgumentException e) {
    ...
    public static BaseComponent[] formatPrefix(final String message) {
        try {
            // Replace the quote as the message formatter does
            return BAT.convertStringToComponent(getString(message).replace("''", "'"));
        } catch (final IllegalArgumentException e) {
            return TextComponent.fromLegacyText("");
        }
    }
